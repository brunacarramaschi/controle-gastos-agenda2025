...
# Inicia o webhook handler integrado
from webhook_handler import start_webhook_handler
...
# Configuração do webhook - servidor Flask independente na porta 8080
WEBHOOK_URL = "https://sistema-de-controle-financeiro-2025-brunacarramasc1.replit.app:8080/webhook"
WEBHOOK_EXTERNAL_URL = "https://sistema-de-controle-financeiro-2025-brunacarramasc1.replit.app:8080/webhook"
WEBHOOK_DATA_FILE = 'webhook_data.json'

def load_webhook_history():
    """Carrega histórico de webhooks"""
    try:
...
def save_webhook_history(history):
    """Salva histórico de webhooks"""
    try:
...
def webhook_monitor():
    st.title("🔗 Monitor de Webhook (Externo)")

    st.success(f"""
    **✅ Webhook Externo Ativo!**

    Use **Webhook.site** para receber e inspecionar webhooks de outros sistemas.
    """)

    st.divider()

    st.subheader("🌐 Como Usar Webhook.site")

    with st.expander("📖 Instruções Passo a Passo", expanded=True):
        st.markdown("""
        ### 1️⃣ Acesse o Webhook.site
        - Abra em uma nova aba: **https://webhook.site**
        - O site gerará **automaticamente uma URL única PARA VOCÊ**
        - ⚠️ **IMPORTANTE**: Não compartilhe esta URL com outras pessoas!

        ### 2️⃣ Copie SUA URL única
        - Exemplo: `https://webhook.site/abc-123-def-456`
        - Esta URL é **pessoal**, **única** e **temporária** (válida por 7 dias)
        - 🔒 Seus dados financeiros ficarão visíveis apenas para você

        ### 3️⃣ Use SUA URL para testes
        - Cole SUA URL no campo abaixo
        - Envie dados de teste clicando no botão
        - Veja os dados chegarem em tempo real no SEU painel do Webhook.site

        ### 4️⃣ Integre com outros serviços (Opcional)
        - Use SUA URL em APIs externas (Mercado Pago, PagSeguro, etc.)
        - Configure notificações de sistemas externos
        - Receba dados de transações automaticamente
        
        ### 🔄 Renovação da URL
        - A URL expira em 7 dias
        - Para continuar usando, gere uma nova URL no Webhook.site
        - Cole a nova URL aqui novamente
        """)

    st.divider()

    st.subheader("🧪 Testar com Webhook.site")

    # Campo para inserir URL do Webhook.site
    st.info("🔒 **Privacidade**: Cada usuário deve gerar sua própria URL no Webhook.site para manter seus dados seguros e isolados.")
    
    webhook_site_url = st.text_input(
        "Cole aqui SUA URL única do Webhook.site",
        placeholder="https://webhook.site/sua-url-unica-aqui",
        help="Acesse https://webhook.site e copie a URL única gerada especialmente para você"
    )

    # Botão para enviar teste ao Webhook.site
    if webhook_site_url and webhook_site_url.startswith("https://webhook.site/"):
        col1, col2 = st.columns(2)

        with col1:
            valor_teste = st.number_input("Valor do teste (R$)", min_value=0.01, value=150.00, step=0.01)
            categoria_teste = st.selectbox("Categoria", ["Venda Online", "Pagamento Recebido", "Assinatura", "Outro"])

        with col2:
            descricao_teste = st.text_area("Descrição do teste", "Teste de integração via Webhook.site")

        if st.button("📤 Enviar Teste para Webhook.site", type="primary"):
            import requests
            try:
                payload_teste = {
                    "tipo": "receita",
                    ...
                    "origem": "Sistema Financeiro - Teste",
                ...
                if response.status_code == 200:
                    st.success(f"""
                    ✅ **Dados enviados com sucesso para Webhook.site!**

                    Agora:
                    1. Volte para a aba do Webhook.site
                    2. Você verá os dados recebidos no painel
                    3. Inspecione headers, corpo JSON, timestamp, etc.
                    """)

                    with st.expander("📦 Ver dados enviados"):
                        st.json(payload_teste)
                else:
                    st.warning(f"Resposta: {response.status_code}")

            except Exception as e:
                st.error(f"❌ Erro ao enviar: {str(e)}")

    elif webhook_site_url:
        st.warning("⚠️ URL inválida. Certifique-se de usar uma URL do Webhook.site começando com `https://webhook.site/`")

    else:
        st.info("👆 Cole a URL do Webhook.site acima para começar a testar")
...
def main():
    ...
    menu = st.sidebar.radio(
        ...
        ["🏠 Dashboard", "➕ Cadastrar Transação", "📊 Análise Financeira",
         "📅 Gerenciar Agenda", "🔗 Webhook Externo", "📄 Relatórios e PDF"]
    )
